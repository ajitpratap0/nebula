...
func (p *PlatformAuthService) GetGoogleAdsCredentials(accountID string) (*GoogleAdsCredentials, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&tokenResp); err != nil {
		return nil, errors.Wrap(err, errors.ErrorTypeData, "failed to decode response")
	}
...
func (p *PlatformAuthService) GetPlatformCredentials(accountID, platform string) (interface{}, error) {
	switch platform {
	case "GOOGLE":
		return p.GetGoogleAdsCredentials(accountID)
	default:
