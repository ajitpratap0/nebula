...
func (pc *ParallelCompressor) CompressData(data []byte) ([]byte, error) {
	...
	chunkChan := make(chan struct{ id int; data []byte }, pc.numWorkers)
	...
	go func() {
		for i, chunk := range chunks {
			select {
			case chunkChan <- struct{ id int; data []byte }{i, chunk}:
			case <-pc.ctx.Done():
				close(chunkChan)
				return
			}
...
func (pc *ParallelCompressor) DecompressData(data []byte) ([]byte, error) {
	...
	offset := headerSize
	chunks := make([]struct{ id int; data []byte }, 0, numChunks)
	...
	for i := 0; i < numChunks; i++ {
		...
		chunks = append(chunks, struct{ id int; data []byte }{
			id:   i,
...
func (pc *ParallelCompressor) compressionWorker(
	chunkChan <-chan struct{ id int; data []byte },
